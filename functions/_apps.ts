...
CPU: 4-16 Kerne`,
      upgradeDescription:
        `v2.0.8: Pinned to olares-worker (RTX 5090). Removed --kv-offloading-size + --quantization. FP8-only AWQ at 200k. v2.0.7: Removed --kv-offloading-size + --quantization. v2.0.6: Reverted --kv-cache-dtype-skip-layers. 200k FP8-only. v2.0.5: Reduce MAX_MODEL_LEN. v2.0.4: Bump GPU_MEMORY_UTILIZATION. v2.0.3: Force re-sync. v2.0.2: Renamed. v2.0.0: Complete restructure. Renamed from aimllmgemma4vision to aimllmgemma4vllm. Switched from bg-digitalservices/Gemma-4-26B-A4B-it-NVFP4 to cyankiwi/gemma-4-26B-A4B-it-qat-AWQ-INT4 with MTP speculative decoding. New image vllm/vllm-openai:v0.23.0. Added triton_attn backend, fp8 KV cache, gemma4 tool calling. Removed NVFP4 patches (gemma4.py, fused_moe_layer.py).`,
...
Disk: 50 GB for model cache`,
    ...
    spec: {
      type: "app",
      entrance: [
        { name: "aimembqwen3vino", title: { en: "Embedder" }, port: 9997, host: "aimembqwen3vino", authLevel: "internal", openMethod: "window" },
      ],
      permission: [],
      middleware: [],
