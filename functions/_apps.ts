...
- Agentic Tool-Calling: 100% (9/9)`,
      upgradeDescription:
                 `v2.4.0: HAMi GPU allocation fix — removed runtimeClassName: nvidia (bypassed HAMi, caused "No compatible compute resources" error). beellama.cpp v0.3.2 → v0.4.1 (native KVarN + DFlash CUDA kernels, reduced checkpoint stalls). Threads 12→16, cache-ram 0→-1 (prompt cache reaktiviert), parallel 1→2 (2x throughput). v2.3.3: Revert --parallel 2→1, --cache-ram 2048→0 (prompt cache aus — OOM-Risiko bei 2x 200k). Sauberes Release basierend auf v2.3.0 (check-auth command:["true"]). v2.3.0: Fix Olares Webhook compat — check-auth uses command:["true"] so app-service webhook patch is harmless. v2.2.6: Fix HAMi compat — removed nvidia.com/gpumem, check-auth args -it→-t 120, removed CUDA_DEVICE_MEMORY_LIMIT. v2.2.5: --parallel 2 + --cache-ram 2048. v2.0.0: Cleanup — removed reasoning-budget-message, min-tokens 256→512.`,
      categories: ["LLM Chat"],
      developer: "Aimighty",
...
v1.1.3: New description + llama.cpp icon.`,
  ...
  },
  {
    metadata: {
      ...
      description: { en: "Wings for Hermes — AI Agent Web UI with agent source fix, auto-install, and theme overhaul" },
      fullDescription:
