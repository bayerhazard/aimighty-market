...
CPU: 4-16 Kerne`,
      upgradeDescription:
        `v2.1.5: Olares-Dependency auf >=1.12.6-0 (apiVersion v3 erfordert Olares 1.12.6+, app-service lehnt >=1.12.2-0 ab). v2.1.4: Migrate OlaresManifest v2 → v3 (app-service lehnt apiVersion v2 ab, HTTP 403). v2.1.3: Cluster-Compat-Fixes — runtimeClassName: nvidia entfernt (bypassed Olares-HAMi), nodeSelector olares-worker entfernt (Node heißt "olares"), Chart dem Market hinzugefügt. v2.1.0: Fix Helm ownership metadata on ConfigMap. v2.0.9: ConfigMap für env vars. v2.0.8: FP8-only AWQ at 200k. v2.0.2: --kv-cache-dtype-skip-layers sliding_window + --calculate-kv-scales. v2.0.0: Restructure — cyankiwi/gemma-4-26B-A4B-it-qat-AWQ-INT4 + MTP spec decoding, vLLM v0.23.0, triton_attn, fp8 KV, gemma4 tool calling.`,
      categories: ["LLM Chat"],
      developer: "Aimighty",
...
Disk: 50 GB for model cache`,
      upgradeDescription:
        `v1.6.8: Olares-Dependency auf >=1.12.6-0 (apiVersion v3 erfordert Olares 1.12.6+, app-service lehnt >=1.12.2-0 ab). v1.6.7: Migrate OlaresManifest v2 → v3 (app-service lehnt apiVersion v2 ab, HTTP 403). v1.6.6: Version alignment + sauberes Chart-Packaging (vorher Repo-Root gezippt). v1.6.5: Fixed deployment title to 'Qwen3 Embedding'. v1.6.4: Renamed to 'AIM Qwen3 Embedding'. v1.6.3: Category update. v1.6.2: Version bump, simplified deployment (Recreate), fixed pod anti-affinity deadlock. v1.5.5: Release v1.5.5. Final naming corrections and entrance status fix. v1.5.1: Fixed app title. v1.5.0: Version bump to force Olares re-sync.`,
