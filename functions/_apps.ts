...
- **Local & Secure** – full data sovereignty; emails and AI models remain exclusively local on your device.`,
        upgradeDescription:
                    `v26.09.111: Cross-Action KI-Followups (Gegenvorschlag) — (1) Bei Termin-Konflikten wird der vom Nutzer gewaehlte Alternativ-Slot jetzt gemerkt. (2) Neue Aktion "Alternative per Mail vorschlagen": erzeugt eine konkrete Gegenvorschlag-E-Mail, die genau den gewaehlten Alternativ-Termin vorschlaegt (statt eines generischen Entwurfs mit zwei Optionen). (3) Slot-Auswahl: Klick auf einen Alternativ-Slot waehlt ihn aus (gold markiert); ein separater "Termin eintragen"-Button bucht den gewaehlten Slot. Built for Olares 1.12.6.
    v26.09.110: KI-Vorschläge-Layout + Followup-Dedup + Settings-Sidebar — (1) KI-Vorschläge-Block im Mail-Bereich: Höhe jetzt exakt wie der untere Sidebar-Block (107px, Abstand Trennlinie → untere Fensterkante), keine inneren Trennlinien, kompakte Darstellung (Aktions-Button direkt neben dem Vorschlag statt am Rand). (2) Followup-Dedup: Bereits ausgeführte KI-Aktionen (Aufgabe anlegen, Termin eintragen, Entwurf öffnen) werden beim erneuten Öffnen der Mail oder nach einem App-Neustart nicht erneut angeboten — die Erinnerung wird im Browser gespeichert. (3) Einstellungen → Cache-Verwaltung: Neuer Button "KI-Aktionen löschen" (analog zu KI-Zusammenfassungen), setzt die Dedup-Erinnerung zurück. (4) Settings-Sidebar: Logo-Bereich und Modul-Icons entfernt, Sidebar jetzt resizierbar wie in den anderen Modulen. Built for Olares 1.12.6.
    v26.09.109: Design-Konsistenz + vollständige englische Sprachversion — (1) Einheitliches Modul-Design: Resizierbare Sidebar (per Drag am Rand) in Mail, Kalender, Kontakte und Aufgaben; einheitliche Modul-Header (gleiche Höhe und Abstände). (2) Gemeinsame Suchfeld-Komponente in allen Modulen (einheitliches Look & Feel). (3) Schriftgrad-Design-Tokens statt fester Pixelwerte; Farbtokens vereinheitlicht (Semantik-Farben für Termin-Akzeptanz/Ablehnung, Gold-Tokens). (4) Vollständige englische Sprachversion (EN) für Kalender, Kontakte, Aufgaben und den globalen Assistenten — Sprachumschalter DE/EN auf dem Startbildschirm. Built for Olares 1.12.6.
    v26.09.108: Robustheit & Performance (Code-Review) — (1) FIX: Beim Oeffnen von Mails mit groesseren Anhaengen blockiert der Dateilesevorgang nicht mehr die gesamte Datenbank (alle anderen API-/Sync-Operationen laufen parallel weiter). (2) FIX Reply-All: Die eigene Absenderadresse wird nicht mehr in den Verteiler aufgenommen. (3) KI-Requests warten jetzt begrenzt (max. 30 s) auf die KI-Engine und zeigen "gerade ausgelastet" statt endlos zu hängen. (4) Sicherheit: Externe Links (open-url) werden nur für http/https geöffnet. (5) /health ist jetzt eine echte Readiness-Pruefung (Datenbank-Check). Built for Olares 1.12.6.
    v26.09.107: Automatische KI-Follow-ups — (1) Beim Oeffnen einer Mail erkennt die KI im Hintergrund Termin-Anfragen und weitere To-dos (ohne manuellen Button, Mail bleibt während der Analyse lesbar). (2) Die Vorschläge erscheinen als Einzelaktionen in einem fixen Footer im Mail-Bereich: Termin eintragen, Antwort/Bestätigung als Entwurf öffnen (vom Nutzer prüfen/bearbeiten/versenden) oder Aufgabe anlegen. (3) Bei Termin-Konflikten werden alternative freie Zeitfenster vorgeschlagen. (4) Der Assistent findet Kontakte jetzt auch über eine interne Suche (robuster) und zeigt für jede ausgeführte Aktion ein Ergebnis an. (5) VCard-Korrektur: Vor-/Nachname wurden vertauscht gespeichert. Built for Olares 1.12.6.
    v26.09.106: Assistent-Kontext + Kontakte-Verknuepfungen — (1) Der Assistent behaelt jetzt den Kontext ueber mehrere Nachrichten eines Dialogs (z.B. "schreibe eine Mail" → "an welche Adresse?" → "marc@example.com" bleibt zusammenhaengend). (2) Der Assistent kennt jetzt auch Telefonnummern aus den Kontakten. (3) Kontakte: Klick auf eine Telefonnummer loest einen Anruf aus (tel:-Link). (4) Kontakte: Klick auf eine Mailadresse oeffnet das Compose-Fenster im Mail-Modul mit vorausgefuelltem Empfaenger (statt externem Mailprogramm). (5) Logo-Hover-Effekt in Aufgaben, Kontakte und Kalender entfernt. Built for Olares 1.12.6.
    v26.09.105: Assistent modul-uebergreifend + Aktionen direkt — (1) Der Assistent (FAB) hat jetzt Zugriff auf die Daten aller Module (Termine der naechsten 7 Tage, Kontakte, letzte Mails) und beantwortet Fragen modul-uebergreifend, z.B. aus dem Mail-Modul "Habe ich morgen Termine?" oder "Wie lautet die Mailadresse von Marc?". (2) Erkannte Aktionen (z.B. "Mail an X beantworten") werden jetzt direkt ausgefuehrt, ohne dass der Befehl als Button/Output im Chat-Dialog angezeigt wird. (3) Mustertext (Placeholder) in den Suchfeldern von Kontakte, Kalender und Aufgaben jetzt vertikal zentriert. Built for Olares 1.12.6.
...
v26.09.94: FIX AI-Code-Review: (1) Phishing-Erkennung aktiv — eingehende Mails werden jetzt beim Sync automatisch auf Betrugsmuster geprueft (Dringlichkeits-Sprache, Link/URL-Mismatch, versteckte Links, verdaechtige TLDs, Zugangsdaten-Bitten, URL-Shortener); Warnhinweis "Phishing-Verdacht" erscheint direkt in der Mail-Liste (Schwelle >0.6) — lief vorher nie, da der Score nie berechnet wurde. (2) Lernschleife repariert — der individuelle Schreibstil pro Empfaenger (Style-Fingerprint) wird jetzt korrekt synthetisiert und beim Verfassen/Beantworten in den KI-Prompt eingespeist (vorher Account-ID-Mismatch → Fingerprint war nie verfuegbar). (3) Tonfall-Profile-Export verdrahtet (POST /ai/tone-profiles/export). (4) Weihnachts-Gruss-Anlass im Prompt korrigiert. Built for Olares 1.12.6.
v26.09.93: NEU Mails zwischen Konten verschieben — Drag & Drop auf den Kontonamen (INBOX) eines anderen Kontos funktioniert jetzt (vorher ohne Reaktion), ebenso auf gleichnamige Unterordner anderer Konten; Raw-IMAP-Pfade werden fuer Quelle UND Ziel korrekt aufgeloest (verschachtelte/nicht-ASCII-Ordner). Der Verschieben-Button bietet jetzt Ziele aller Konten gruppiert nach Konto (Mehrfachauswahl wird nacheinander verschoben), das Nachrichten-Kontextmenü hat einen Verschieben-Eintrag. PERF Sofortiger Ordnerwechsel — gecachte Ordner erscheinen sofort, die Aktualisierung laeuft unsichtbar im Hintergrund (kein Skeleton/Spinner mehr bei bereits sichtbarer Liste); Server-Cache-TTL 5s→30s mit expliziter Invalidierung nach Move/Delete/Flag/Gelesen; Client-Freshness-Fenster 15s gegen Redundanzt-Fetches beim schnellen Ordnerwechsel. Built for Olares 1.12.6.
